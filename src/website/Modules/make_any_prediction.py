...
class New_Input(object):
    ...
    **Output**
    ...
    def __init__(self,hike,date,condition='condition|snow'):
        ...
        df = pd.read_csv('static/files/olympics_merged.csv', sep = '|',lineterminator='\n')
        self.df = df.fillna(0)
        #self.df_trail = pd.read_csv('../data/WTA_trails_clean_w_medians.csv',lineterminator='\n')
        self.df_trail = pd.read_csv('static/files/WTA_trails_clean_w_medians.csv',lineterminator='\n')
    ...
    def get_train_df(self):
        ...
        X_all = pd.read_csv('static/files/olympics_Xall.csv',sep = '|',lineterminator='\n')
        self.X_train= X_all.drop(new,axis=1)
        #y_all = pd.read_csv('../data/olympics_yall.csv',sep = '|',lineterminator='\n')
        y_all = pd.read_csv('static/files/olympics_yall.csv',sep = '|',lineterminator='\n')
